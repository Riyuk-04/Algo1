...
void insert(Node *head, int ascii, int freq){				//Used in the formation of ordered linked list
	Node *temp1 = head;
...
void createBST(Node **head, int *index){					//Creates the tree once the linked list is formed 
	Node *temp1 = *head;
	if(temp1->next == NULL)
		return;
...
void pre_order(Node *head){									//prints the pre order traversal in ther tree
	if(head == NULL)
		return;
...
void find(int ascii, int freq, Node *head){					//function to find the coe for an element
	while(1){
