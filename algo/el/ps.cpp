...
Point* merge(Point* left, Point* right){
	int size = (sizeof(left) + sizeof(right))/sizeof(Point);
	//cout<<"size = "<<size;
	int h1,h2,l,r,index;
	h1 = 0;
	h2 = 0;
...
Point* split(Point* points, int size){
	if (size == 1){
		...
		Point* lefty = split(points, size/2);	cerr<<"left "<<lefty.size();
		Point* righty = split(points + size,size-size/2);	cerr<<"right ";
		//Point net[] = merge(lefty,righty);	cerr<<"net ";
		//cerr<<sizeof(net);
...
/*Here i am converting the boxes format into the outline format as point*/
	...
	cerr<<"hello ";
	point_array = split(point_array, size);
	outputsize = (sizeof(point_array))/sizeof(Point);
